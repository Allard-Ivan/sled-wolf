...
function createIFrame(w, h, parentName, playerElement, vid, partnerId, password, autoplay, events) {
    if (YKP.isPC) {
        // var iframes=document.getElementById(parentName + '').getElementsByTagName("iframe");
        var iframes = playerElement.getElementsByTagName("iframe");
...
var YoukuPlayerSelect = function(params) {
    ...
    this.playerElement = params.playerElement;
...
YoukuPlayerSelect.prototype = {
	...
	selectPCH5:function() {
		...
		createIFrame(cw, ch, self._target, self.playerElement, self._vid, self._partnerId, self._password, self._autoplay, self._events);
 ...
 *
 */
YKU.Player = function(target, playerElement, config) {
